class Solution:
    def groupAnagrams(self, strs: List[str]) -> List[List[str]]:
        ...
        Time Complexity: O(m * n) - we iterate through the input list of length n. We need to go through char of each string so it would be O(m * n) where m is the len of the longest string. 
        Space Complexity: O(m * n)- need to store all strings in the input to the hashmap, which takes O(n) and m is the longest string -> O(m*n) space is used.
