     ...
     speed                         2 
          ...
          Can we iterate through this new array to get the number of fleets?
               - At each iteration through this new array,
                    ...
                    We do need to keep the ones in that take longer and expel ones that are equal/smaller than the one we're checking (if the one we're checking has a smaller time to target, then this car will be th one that holds everything up)

          We can sort the new array by desc position -> use a monotonic stack to keep time to target as values. The slower cars closer to the target block the cars in the back (no matter how fast the ones are in the back)
